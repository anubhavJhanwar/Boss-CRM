...
const EditClientScreen: React.FC<{ navigation: any; route: any }> = ({ navigation, route }) => {
  const { clientId } = route.params;
  const { selectedClient, fetchClientById, updateClient, isLoading } = useClientStore();
  ...
  useEffect(() => {
    if (selectedClient) {
      setFormData({
        ...
        paymentMode: selectedClient.paymentMode,
        amountPaid: selectedClient.amountPaid.toString(),
        subscriptionMonths: selectedClient.subscriptionMonths.toString(),
        notes: selectedClient.notes || ''
      });
      
      if (selectedClient.manualStartDate) {
        setUseManualDate(true);
        setManualStartDate(selectedClient.manualStartDate.split('T')[0]);
      }
    }
  }, [selectedClient]);
